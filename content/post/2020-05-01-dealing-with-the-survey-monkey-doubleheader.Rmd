---
title: Dealing With The Survey Monkey Doubleheader
author: Matt Roumaya
date: '2020-05-02'
...
**Let's start by viewing our response data. You'll need to use the packages `tidyverse`, `janitor`, and `readxl` if you're using a *.xlsx* file.** 
...
3. `pivot_longer(everything())` moves all of our column *names* into the first column as values, and puts all of our secondary column names into the second column.     
...
4. `group_by(grp = cumsum(!startsWith(name, "...")))` groups rows and then applies a cumulative sum for all values in the `name` column that do not start with "..." until a value other than "..." is encountered. This is better shown in the table below:  
...
Now if we want to subset data based on cerain questions/columns, we can do it really easily using `starts_with` and `ends_with`. Sometimes it's easier to rename columns so that they're descriptive enough, and sometimes it's fine to keep some really long descriptive column names if your survey contains a lot of similar questions. 
